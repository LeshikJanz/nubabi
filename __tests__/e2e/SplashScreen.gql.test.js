...
describe("E2E GraphQL", () => {
  test("SplashScreen GQL", done => {
    const queryString = fs.readFileSync(
      path.resolve(__dirname, "../../__generated__/SplashScreen.graphql"),
      "utf-8"
    );

    const query = gql`${queryString}`;
    const variables = {
      hasCurrentBaby: false,
    };

    apollo
      .query({ query, variables })
      .then(resp => {
        expect(resp.data).toHaveProperty('viewer.randomQuote.id');
        expect(resp.data).toHaveProperty('viewer.randomQuote.text');
        done();
      })
      .catch(err => done(err));
  });
