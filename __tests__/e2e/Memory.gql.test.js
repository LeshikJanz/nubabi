...
describe("E2E GraphQL", () => {
  test("Memory GQL", done => {
    const queryString = fs.readFileSync(
      path.resolve(__dirname, "../../__generated__/Memory.graphql"),
      "utf-8"
    );

    const query = gql`${queryString}`;
    const variables = {
      id: "TWVtb3J5Oi1MMEF0c2U2Z2lXRlAwcmJlNXZy",
      babyId: process.env.NUBABI_E2E_BABY_ID,
    };

    apollo
      .query({ query, variables })
      .then(resp => {
        expect(JSON.parse(JSON.stringify(resp.data))).toMatchSnapshot("Memory");
        done();
      })
      .catch(err => done(err));
  });
