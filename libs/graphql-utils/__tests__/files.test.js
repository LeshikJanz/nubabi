...
describe('graphql-utils/files', () => {
  describe('getTypenameForFile', () => {
    it('returns the typename for a file given a content-type', () => {
      expect(getTypenameForFile({ contentType: 'image/jpeg'})).toEqual('Image');
    ...
    it('returns GenericFile for content-types it does not recognize', () => {
      expect(getTypenameForFile({ contentType: 'application/octet-stream'})).toEqual('GenericFile');
      expect(getTypenameForFile({ contentType: 'application/zip'})).toEqual('GenericFile');
      expect(getTypenameForFile({ contentType: 'text/plain'})).toEqual('GenericFile');
    })
  });
