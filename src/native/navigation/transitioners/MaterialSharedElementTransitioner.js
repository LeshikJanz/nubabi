...
class MaterialSharedElementTransitioner extends Component {
  ...
  getChildContext() {
    ...
    return {
      registerSharedView(sharedItem: SharedItem) {
        ...
        const matchingItem = self.state.sharedItems.findMatchByName(name,
          containerRouteName);
  ...
  _render(
    props: NavigationTransitionProps, prevProps: NavigationTransitionProps) {
    const scenes = props.scenes.map(
      scene => this._renderScene({ ...props, scene }));
  ...
  _getSharedElementStyle(props, prevProps, itemFrom, itemTo) {
    ...
    const animateFontSize = (itemFrom, itemTo) => {
      ...
      const getFontSize = element => (element.props &&
  ...
  _renderOverlay(
    ...
    const pairs = this.state.sharedItems.getMeasuredItemPairs(fromRoute,
      toRoute);
    const sharedElements = pairs.map((pair, idx) => {
      const { fromItem, toItem } = pair;
      const animatedStyle = this._getSharedElementStyle(props, prevProps,
        fromItem, toItem);
  ...
  _renderScene(transitionProps) {
    ...
    return (
      <Animated.View key={transitionProps.scene.route.key}
